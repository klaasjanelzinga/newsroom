...
async def test_change_display_name(repositories: MockRepositories, faker: Faker, user: User, bearer_token: str):
    ...
    avatar = await fetch_avatar_image(bearer_token)
...
async def test_change_display_name_authorization(
    repositories: MockRepositories, faker: Faker, user: User, bearer_token: str
):
...
@pytest.mark.asyncio
async def test_keep_avatar_but_dont_send(repositories: MockRepositories, faker: Faker, user: User, bearer_token: str):
    new_name = faker.name()
    new_avatar = faker.paragraph()
    update_profile_request = UpdateUserProfileRequest(
        display_name=new_name, avatar_image=new_avatar, avatar_action="keep"
    )
    await update_profile(update_profile_request, bearer_token)
