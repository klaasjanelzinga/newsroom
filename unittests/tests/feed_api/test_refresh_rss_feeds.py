...
async def test_refresh_with_duplicate_titles(faker: Faker, repositories: MockRepositories, user: User):
    ...
    assert feed.feed_id in user.subscribed_to
    assert repositories.news_item_repository.count() == 5
    assert user.number_of_unread_items == 5
    ...
    assert len(feed_items[0].alternate_links) == 3
    assert len(feed_items[0].alternate_links) == len(feed_items[0].alternate_title_links)
    # These titles below were aggregated in the FeedItem above.
    assert "'Grote brand' in boerderij aan de Stadsweg (update)" in feed_items[0].alternate_title_links
    assert "Metershoge vlammen slaan uit boerderij aan Stadsweg (update)" in feed_items[0].alternate_title_links
    assert "Metershoge vlammen slaan uit boerderij aan Stadsweg" in feed_items[0].alternate_title_links
    assert feed_items[0].title.startswith("[Updated]")
    # Item with no similarities
    feed_items = [
        item
        for item in repositories.feed_item_repository.store.values()
    ...
    assert len(feed_items) == 1
    assert len(feed_items[0].alternate_title_links) == 0
    assert len(feed_items[0].alternate_links) == 0
    assert not feed_items[0].title.startswith("[Updated]")
    ...
    await refresh_rss_feeds()
    assert repositories.feed_item_repository.count() == 5
    assert repositories.news_item_repository.count() == 6  # Created an updated news-item
    assert user.number_of_unread_items == 6
    feed_items = [
        item
        for item in repositories.feed_item_repository.store.values()
        if item.link.startswith("https://www.oogtv.nl/2021/01/uitslaande-brand-in-boerderij-in-ulgersmaborg/")
    ...
    assert len(feed_items) == 1
    assert len(feed_items[0].alternate_title_links) == 1
    assert len(feed_items[0].alternate_links) == 1
    assert feed_items[0].title.startswith("[Updated]")
    ...
    await refresh_rss_feeds()
    assert repositories.feed_item_repository.count() == 5
    assert repositories.news_item_repository.count() == 6
    assert user.number_of_unread_items == 6
