...
async def test_unsubscribe(
    ...
    response = await unsubscribe_from_feed(feed_id=feed.feed_id, authorization=bearer_token)
...
async def test_unscubscribe_while_already_unscubscribed(
    faker: Faker, user: User, feed: Feed, repositories: MockRepositories, bearer_token: str
):
    number_of_subscriptions = feed.number_of_subscriptions
