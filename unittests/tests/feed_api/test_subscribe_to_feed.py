...
async def test_subscribe(
    faker: Faker, user: User, feed: Feed, feed_items: List[FeedItem], repositories: MockRepositories, bearer_token: str
):
    assert feed.feed_id not in user.subscribed_to
    number_of_subscriptions = feed.number_of_subscriptions
    response = await subscribe_to_feed(feed_id=feed.feed_id, authorization=bearer_token)
