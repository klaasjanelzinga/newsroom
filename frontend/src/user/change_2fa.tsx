...
class ChangeTotp extends React.Component<ChangeTotpAttrs, ChangeTotpState> {
    ...
    async start_totp_registration(): Promise<void> {
        this.api.post<OTPRegistrationResponse>("/user/start-totp-registration", JSON.stringify({}))
    ...
    async disable_totp(): Promise<void> {
        this.api.post<UserResponse>("/user/disable-totp", JSON.stringify({}))
                        ...
                        <Grid container>
                            ...
                            {this.state.generated_secret && <Grid container>
                                    <Grid item sm={12}>
                                        ...
                                        secret <pre>{this.state.generated_secret}</pre>
                                        to seed the authenticator app (manually).
                                    </Grid>
                                    <Grid item sm={12}>
                                        Keep these backup codes in a safe place:
                                        <pre>
                                                {this.state.backup_codes.map(code => <div key={code}>{code}</div>)}
                                            </pre>
                                    </Grid>
                                    <Grid item sm={12}>
                                        <Grid item sm={6}>
