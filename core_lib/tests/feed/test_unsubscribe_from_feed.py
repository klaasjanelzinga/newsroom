...
def test_unsubscribe(subscribed_user: Tuple[User, Feed], repositories: Repositories):
...
def test_unscubscribe_while_already_unscubscribed(user: User, feed: Feed, repositories: Repositories):
    assert feed.feed_id not in user.subscribed_to
    unsubscribe_user_from_feed(user=user, feed=feed)
    assert feed.feed_id not in user.subscribed_to
